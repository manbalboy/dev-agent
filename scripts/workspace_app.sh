...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/workspace_app.sh start  --app <app_code> [--repo owner/repo] [--install]
  bash scripts/workspace_app.sh stop   --app <app_code>
  bash scripts/workspace_app.sh status --app <app_code>

Behavior:
  - Auto-assigns and persists app port in 3100-3199 range.
...
start_app() {
  ...
  if [[ -f "$pid_file" ]]; then
    ...
    if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
      echo "Already running: app=$app pid=$old_pid port=$port"
  ...
  echo "[INFO] Starting app=$app on port=$port"
  (
    cd "$repo_path"
  ...
  echo "Started: app=$app pid=$new_pid port=$port"
  echo "URL: http://$(hostname -I | awk '{print $1}'):$port"
  echo "Log: $log_file"
}
...
status_app() {
  ...
  if [[ -f "$pid_file" ]]; then
    ...
    pid="$(cat "$pid_file" 2>/dev/null || true)"
    if [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null; then
      echo "RUNNING app=$app pid=$pid port=$port"
      exit 0
    fi
  fi
  echo "STOPPED app=$app port=$port"
}
